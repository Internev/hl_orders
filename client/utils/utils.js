...
export const parseStoreGeo = (csv) => {
  ...
  return rows.filter(s => s[1]).map(store => {
    ...
    let storeObj = {
      ...
      name: store[1].trim(),
      street: street.trim(),
      suburb: store[len - 6].trim(),
      state: store[len - 5],
      postcode: store[len - 4],
      comment: store[len - 3],
      lat: Number(store[len - 2]),
      lng: Number(store[len - 1])
    }
    return storeObj
  })
}
